...
import me.furt.projectv.libnoise.Misc;
...
public class Curve extends ModuleBase {
    ...
    public int findInsertionPos(double inputValue) throws ExceptionInvalidParam {
        ...
        for (insertionPos = 0; insertionPos < controlPointCount; insertionPos++) {
            if (inputValue < controlPoints[insertionPos].inputValue) // We found the array index in which to insert the new control point.
            // Exit now.
            {
                break;
            } else if (inputValue == controlPoints[insertionPos].inputValue) // Each control point is required to contain a unique input value, so
            // throw an exception.
            {

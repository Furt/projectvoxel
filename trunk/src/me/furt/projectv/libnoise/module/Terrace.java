...
public class Terrace extends ModuleBase {
    ...
    public int findInsertionPos(double value) throws ExceptionInvalidParam {
        ...
        for (insertionPos = 0; insertionPos < controlPointCount; insertionPos++) {
            if (value < controlPoints[insertionPos]) // We found the array index in which to insert the new control point.
            // Exit now.
            {
                break;
            } else if (value == controlPoints[insertionPos]) // Each control point is required to contain a unique value, so throw
            // an exception.
            {
